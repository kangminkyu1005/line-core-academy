...
test("the writing order replaces the role blueprint and stays above the keyword guide", () => {
  const sequence = source.indexOf("<WritingSequence guide={mission.gradingGuide}/>");
  const keywords = source.indexOf("<MissionKeywords guide={mission.gradingGuide}/>");
  const editor = source.indexOf('<div className="editor">', sequence);
  assert.ok(sequence >= 0 && sequence < keywords && keywords < editor);
  assert.doesNotMatch(source, /코드 역할 설계도|ROLE BLUEPRINT/);
  assert.match(source, /aria-label="이번 미션의 코드 작성 순서"/);
  assert.doesNotMatch(source, /위에서 아래로 차례대로 입력해요/);
  assert.equal(source.match(/className="keywordGroup keywordSequence"/g)?.length ?? 0, 0);
  assert.match(source, /aria-label="이번 문제에 필요한 정확한 키워드와 채점 약속"/);
});
...
test("player entry reuses the title scene and Korean text does not split mid-word", () => {
  ...
  assert.match(styles, /\.loginGame :where\(\.loginLogo,\.backTitle,\.loginSound,\.loginCharacter,\.gameLoginCard,\.loginModules\)\{position:fixed\}/);
...
test("dialogue highlights concepts and the BGM mix is clearly raised", () => {
  assert.match(source, /className="dialogueKeyword"/);
  assert.match(styles, /\.dialogueKeyword\{color:#ffd95c/);
  assert.match(source, /noteStart,1\.26,\.06,"sine",bgmBus/);
  assert.match(source, /noteStart,1\.3,\.022,"triangle",bgmBus/);
});

test("passing code opens mission clear without an intermediate confirmation", () => {
  const runCode = source.slice(source.indexOf("function runCode()"), source.indexOf("function finishMission()"));
