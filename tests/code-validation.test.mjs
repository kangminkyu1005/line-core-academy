...
test("canonical generated code passes every one of the five practice validators",()=>{
  const programs=[
  ...
  programs.forEach((program,index)=>{
    ...
    const result=validateCode(missionId,program);
    assert.equal(result.passed,true,`mission ${missionId}: ${result.missing.join("\n")}`);
  });
});
