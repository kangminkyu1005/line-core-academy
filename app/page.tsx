...
import { CodeMissionExperience } from "./code-screen-preview/page";
...
const missions: Mission[] = [
  ...
  },
  {
    ...
    conceptChecks: [
      {badge:"BASE SPEED",question:"BASE_SPEED가 의미하는 값은 무엇일까요?",options:["보정이 없을 때 두 모터가 앞으로 가는 기본 속도", "검정색 반사광", "이전 오차", "센서가 읽는 현재값"],answer:0,feedback:"BASE_SPEED는 함수 밖에서 준비하는 기본 속도 설정값입니다. 함수가 실행되면 소문자 매개변수 base_speed로 전달됩니다.",codeClue:"BASE_SPEED에는 수업 로봇에 안전한 1~100 사이 속도를 직접 선택해 저장해요."},
      {badge:"P GAIN",question:"KP를 크게 하면 가장 먼저 달라지는 것은 무엇일까요?",options:["현재 오차에 대한 보정 반응이 커진다", "기준값이 자동으로 바뀐다", "반복문이 느려진다", "센서가 흰색만 읽는다"],answer:0,feedback:"KP는 함수 밖에서 준비하는 P 설정값입니다. 함수 안에서는 매개변수 kp가 이 값을 받아 사용합니다.",codeClue:"KP는 0보다 큰 수로 시작하고 주행 결과를 보며 조절해요."},
      {badge:"D GAIN",question:"KD의 주된 역할은 무엇일까요?",options:["오차가 빠르게 변할 때 추가로 반응해 흔들림을 줄인다", "로봇의 기본 속도를 저장한다", "검정과 흰색의 평균을 구한다", "함수 이름을 만든다"],answer:0,feedback:"KD는 함수 밖에서 준비하는 D 설정값이며, 함수 안의 매개변수 kd로 전달됩니다.",codeClue:"KD는 0 이상으로 선택할 수 있고, 0이면 P 제어만 사용하는 상태가 됩니다."},
    ],
    hint: "변수 이름은 값의 역할을 드러내야 합니다. 따라가려는 목표값을 뜻하는 이름을 찾아보세요.",
    codeTitle: "측정값과 전역 설정값을 직접 선언하세요.", codeHint: "함수 밖에서 준비하는 설정값은 대문자로 쓰고, 숫자는 조건 안에서 직접 선택하세요. TARGET은 두 반사광 설정값의 평균식으로 작성합니다.",
    hintLevels: ["함수 밖의 설정값은 대문자 이름 = 값 형태로 선언해 함수 안의 소문자 매개변수와 구분해요.", "검정 반사광은 흰색보다 작아야 하고 TARGET은 두 값 사이에 있어야 해요.", "TARGET에는 BLACK_VALUE와 WHITE_VALUE를 더한 뒤 2로 나누는 식이 필요해요."],
    gradingGuide: {
      exact: ["BLACK_VALUE", "WHITE_VALUE", "BASE_SPEED", "TARGET", "KP", "KD"],
      sequence: ["반사광 측정", "기본 속도", "기준값", "Kp", "Kd"],
      rule: "BLACK_VALUE < WHITE_VALUE, BASE_SPEED는 1~100, KP는 0보다 크게, KD는 0 이상으로 정해요. TARGET은 두 측정 설정값의 평균식이어야 해요.",
      freedom: "숫자는 하나의 정답이 아니며 허용 범위 안에서 직접 선택할 수 있어요.",
    },
    codeGuide: [["BLACK_VALUE = 20", "검정 바닥에서 측정한 전역 설정값"], ["WHITE_VALUE = 80", "흰 바닥에서 측정한 전역 설정값"], ["BASE_SPEED = 60", "직진할 때의 전역 속도 설정값"], ["TARGET = (BLACK_VALUE + WHITE_VALUE) / 2", "경계를 판단하는 전역 기준값"], ["KP = 0.8", "현재 오차에 반응하는 전역 계수"], ["KD = 0.3", "급격한 흔들림을 줄이는 전역 계수"]],
    guidePatterns: ["BLACK_VALUE = 측정값", "WHITE_VALUE = 측정값", "BASE_SPEED = 안전한 속도", "TARGET = (검정값 + 흰색값) / 2", "KP = P 조절값", "KD = D 조절값"],
    starter: "# 함수 밖에서 사용할 검정 반사광 설정값이에요 (0~100) (아래의 코드 입력)\nBLACK_VALUE = \n\n# 함수 밖에서 사용할 흰색 반사광 설정값이에요 (검정보다 큰 값) (아래의 코드 입력)\nWHITE_VALUE = \n\n# 함수 밖에서 사용할 기본 속도 설정값이에요 (1~100) (아래의 코드 입력)\nBASE_SPEED = \n\n# 두 반사광 설정값의 평균으로 경계 기준값을 계산해요 (아래의 코드 입력)\nTARGET = \n\n# 함수 밖에서 사용할 P·D 계수를 정해요 (아래의 코드 입력)\nKP = \nKD = ", solution: "BLACK_VALUE = 20\nWHITE_VALUE = 80\nBASE_SPEED = 60\nTARGET = (BLACK_VALUE + WHITE_VALUE) / 2\nKP = 0.8\nKD = 0.3",
  ...
  },
  {
    ...
    connection: "1장에서 만든 대문자 설정값 BASE_SPEED, TARGET, KP, KD를 line_follow 함수의 소문자 매개변수로 전달해 볼게요.",
...
function migrateGlobalSettingNames(code:string){
  const names:Record<string,string>={black_value:"BLACK_VALUE",white_value:"WHITE_VALUE",base_speed:"BASE_SPEED",target:"TARGET",kp:"KP",kd:"KD"};
  return Object.entries(names).reduce((next,[previous,current])=>next.replace(new RegExp(`\\b${previous}\\b`,"g"),current),code);
}
...
export default function Home() {
  ...
  useEffect(()=>{
    const restoreSession = window.setTimeout(()=>{
      ...
      const savedBgmVolume = Number(localStorage.getItem("linecore-bgm-volume"));
      if(savedName) setName(savedName);
      if(Number.isFinite(savedBgmVolume)&&savedBgmVolume>=0&&savedBgmVolume<=1)setBgmVolume(savedBgmVolume);
      if(storageVersion!=="2"){
      ...
      try{if(savedCodes){const migratedCodes=JSON.parse(savedCodes).map((item:string,index:number)=>migrateGuidedComments(index===1?migrateGlobalSettingNames(item):item));setStudentCodes(migratedCodes);localStorage.setItem("linecore-codes",JSON.stringify(migratedCodes))}}catch{localStorage.removeItem("linecore-codes")}
  ...
  useEffect(()=>{
    ...
    localStorage.setItem("linecore-bgm-volume",String(bgmVolume));
  },[bgmVolume]);
  useEffect(()=>{
    const bgm=bgmRef.current;
    if(!bgm)return;
    if(!soundOn){bgm.pause();return;}
    if(screen!=="title")void bgm.play().catch(()=>setBgmPlaying(false));
  },[screen,soundOn]);
  ...
  function openMission(id:number,codes=studentCodes,assists=assistHistory){
    ...
    setActive(id);setStep(0);setQuestionRound(0);setChoice(null);setChecked(false);setCode(savedCode);setPassed(false);setShowClear(false);setShowLearningReview(false);setValidationIssues([]);setAttempts(assist.attempts);setWritingMode("guided");setDialogueIndex(0);setDialogueDone(false);setDialogueInstant(false);setBriefingReady(false);setScreen("mission");
  }
  function startGame(){
    audio.activate();audio.play("click");
    if(soundOn)void bgmRef.current?.play().catch(()=>setBgmPlaying(false));
  ...
  function runCode(nextCode=code){
    ...
    const result=validate(active,nextCode,nextAttempt);
    const nextCodes=studentCodes.map((item,index)=>index===active?nextCode:item);
    const nextAssist=assistHistory.map((item,index)=>index===active?{...item,usedSolution:false,attempts:nextAttempt}:item);
    setCode(nextCode);persistLearning(nextCodes,nextAssist);setAttempts(nextAttempt);setPassed(result.passed);setShowClear(result.passed);setValidationIssues(Array.from(new Set(result.missing)));
  ...
  function toggleSound(){
    ...
    if(next){audio.activate();void bgmRef.current?.play().catch(()=>setBgmPlaying(false));window.setTimeout(()=>audio.play("correct"),0)}else bgmRef.current?.pause();
  }
  ...
  const backgroundMusic=<audio ref={bgmRef} src="/assets/line-core-reboot.mp3" loop preload="auto" onPlay={()=>setBgmPlaying(true)} onPause={()=>setBgmPlaying(false)}/>;
  const screenAudioControl=(className:string)=><div className={`screenAudioControl ${className}`}>
    <button className="soundToggle" onClick={toggleSound} aria-pressed={soundOn} aria-label={soundOn?"BGM과 효과음 끄기":"BGM과 효과음 켜기"}><Icon name={soundOn?"sound":"mute"} size={17}/><span>{soundOn?"BGM + 효과음":"음향 꺼짐"}</span></button>
    <label><span>음량</span><input aria-label="배경음 음량" type="range" min="0" max="1" step="0.05" value={bgmVolume} onChange={(event)=>setBgmVolume(Number(event.target.value))}/></label>
  </div>;

  if(screen==="title") return <>{backgroundMusic}<main className="titleScreen" aria-hidden={showGlossary||Boolean(loadingMessage)} inert={showGlossary||Boolean(loadingMessage)}>
    ...
    {screenAudioControl("titleAudioControl")}
  ...
  if(screen==="login") return <>{backgroundMusic}<main className="loginGame" aria-hidden={showGlossary||Boolean(loadingMessage)} inert={showGlossary||Boolean(loadingMessage)}>
    ...
    {screenAudioControl("loginAudioControl")}
  ...
  if(screen==="quiz") return <>{backgroundMusic}<main className="finalQuizScreen" aria-hidden={showGlossary||Boolean(loadingMessage)} inert={showGlossary||Boolean(loadingMessage)}>
  ...
  if(screen==="ending") return <>{backgroundMusic}<main className="endingScreen" aria-hidden={showGlossary||Boolean(loadingMessage)} inert={showGlossary||Boolean(loadingMessage)}>
    ...
    {screenAudioControl("endingAudioControl")}
  ...
  return <>{backgroundMusic}<main className="gameScreen" aria-hidden={showGlossary||Boolean(loadingMessage)} inert={showGlossary||Boolean(loadingMessage)}>
    <header className="gameHud" aria-hidden={showClear} inert={showClear}><img src="/assets/playwell-logo.png" alt="Playwell"/><div className="chapterInfo"><small>{mission.chapter}</small><b>{mission.title}</b></div><div className="chapterProgress"><span>CORE {active+1} / {missions.length}</span><div>{missions.map((_,i)=><i className={i<active?"done":i===active?"active":""} key={i}/>)}</div></div><div className="stageProgress"><small>{stageNames[step]}</small><div>{stageNames.map((s,i)=><i className={i<step?"done":i===step?"active":""} title={s} key={s}/>)}</div></div><div className="hudActions"><GlossaryButton onClick={()=>openGlossary()}/><div className="gameBgmControl"><button className="soundToggle" onClick={toggleSound} aria-pressed={soundOn} aria-label={soundOn?"BGM과 효과음 끄기":"BGM과 효과음 켜기"}><Icon name={soundOn?"sound":"mute"} size={16}/><span>{bgmPlaying&&soundOn?"BGM":"OFF"}</span></button><input aria-label="배경음 음량" type="range" min="0" max="1" step="0.05" value={bgmVolume} onChange={(event)=>setBgmVolume(Number(event.target.value))}/></div><button className="exitGame" onClick={()=>transition(()=>setScreen("title"),"게임을 안전하게 저장하는 중",500)}>게임 종료</button></div></header>
    ...
    <section className={`missionMain ${step===0?"introMode":""}`}>
      ...
      {step===2&&<CodeMissionExperience
        key={`mission-code-${active}`}
        embedded
        missionId={active}
        savedCodes={studentCodes.map((item,index)=>index===active?code:item)}
        onReview={()=>{audio.play("click");setShowLearningReview(true)}}
        onReset={()=>setCode(mission.starter)}
        onSubmit={(nextCode)=>runCode(nextCode)}
      />}
